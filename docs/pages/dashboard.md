...
## Purpose
One glance at how the business is doing today, this week, this month — clean metrics with comparison available on demand.

## User questions this page answers
- Are today's numbers on track vs last week's same-day curve?
- What's my revenue, orders, ad spend, and ROAS for the selected period?
...
- How is the business performing over the selected range?
...
| GA4 | no | first-party GA4 signals on `orders.utm_*` + GA4 event data | real-time per event |
| Real | yes | `RevenueAttributionService` computed from the other five; neutral zinc badge | derived on request; recomputes on filter change (Klaviyo-style retroactive recalc, [UX §6](../UX.md#6-interaction-conventions)) |
...
- `KpiGrid` (4 cols on desktop, 2 on tablet, 1 on mobile) · clean metric cards, best source auto-picked:
  - `MetricCard` "Revenue (28d)" · auto-source=Real · compare-sources popover available ([UX §5.1](../UX.md#51-metriccard--the-primary-data-primitive))
  - `MetricCard` "Orders" · auto-source=Store · compare-sources popover shows platform-reported Purchases
  - `MetricCard` "Ad Spend" · auto-source=blended (Facebook+Google) · compare-sources splits by platform
  - `MetricCard` "MER" · auto-source=Real · label expands first-time-shown as "MER — Marketing Efficiency Ratio"
...
- Two-column region (8/4 split):
  - **Left (8 cols)** — `LineChart` "Revenue over time" · primary line = Real (zinc), secondary lines togglable: Store (slate), Facebook (indigo), Google (amber), GSC (emerald) via legend click — [UX §4 source colors](../UX.md#color-tokens) · `GranularitySelector` defaults Weekly for ≥14d ranges · dotted incomplete-period segment always-on ([UX §5.6](../UX.md#56-chart-primitives)) · `TargetLine` when workspace has a Revenue target set ([UX §5.23](../UX.md#523-target))
...
**Portfolio variant** (3+ workspaces, `?view=portfolio`): all six KPI cards swap to `MetricCardPortfolio` with per-workspace contribution bar + stacked per-workspace sparklines ([UX §5.1.1](../UX.md#511-metriccardportfolio)). LineChart gains one line per workspace (max 5 visible, overflow grouped as "Other").
...
- [Northbeam Overview Home](../competitors/_teardown_northbeam.md#screen-overview-home) — AttributionModel / Window / AccountingMode selectors above the KpiGrid (Nexstage places these in-page per §3 chrome rules), Revenue (1st Time) framing available via `WindowSelector` New Customer filter.
...
- LineChart reduces to the Real line only by default; other sources togglable via legend.
