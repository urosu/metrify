...
- **Sessions** — Session count. Source priority ladder: **Shopify Analytics API (ShopifyQL `FROM sessions SHOW visits, online_store_visitors`) → GA4 Data API (for WooCommerce stores, or any Shopify store that lacks the `read_analytics` scope) → null**. The Nexstage Site first-party pixel is not a v1 session source (deferred to v2). The `daily_snapshots.sessions_source` column (`shopify` | `ga4`) records which provider won for each store-day, and `MetricSourceResolver` surfaces this as the SourceBadge active state on the CVR card.
