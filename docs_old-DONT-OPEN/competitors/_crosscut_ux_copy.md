...
| **Explaining a data source** | Pol: side-by-side columns with platform logo; hover platform logo reveals source description. TW: source chips on metric cards. Put: source badge per transaction row. Rkr: de-dup vs platform toggle. | **Source badge = platform logo + short code (`Real` · `Shop` · `FB` · `GA` · `GSC` · `GA4`)** on every metric in canonical left-to-right order. Hover badge: `Source: Facebook Ads Manager · Last synced 4 min ago · Window: 7d click`. |
