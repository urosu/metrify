...
- Every `MetricCard` in Portfolio scope retains the 6 source badges in canonical order (Real · Store · Facebook · Google · GSC · GA4). Each is a **sum across all workspaces** in reporting currency, with FX conversion applied at transaction-date rates.
