...
|---|---|---|---|
| Real (yellow-400) | Yes — computed. Default lens. | `RevenueAttributionService` output joined to spend | Derivative of above |
...
| GA4 (violet-500) | No | `ga4_daily_attribution` session/conversion data; provides a cross-check on platform-reported conversions | <15 min after nightly sync |
