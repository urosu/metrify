...
| GA4 | no | `ga4_order_attribution` joined to `orders.external_id` via `transaction_id`; enriches `attribution_first_touch` / `attribution_last_touch` when store-side parsers have weaker data. Resolved by `GA4Source` (priority 5 in `AttributionParserService`). | computed at order ingest |
| Real | yes | `RevenueAttributionService`-computed attribution winner per order; gold lightbulb. Default lens. | derived on request |
