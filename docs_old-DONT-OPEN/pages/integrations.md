...
- Toolbar row (right-aligned):
  - Primary button `+ Add integration` → opens modal with the full connector grid (Shopify, WooCommerce, Facebook Ads, Google Ads, GSC, **GA4**; greyed "Coming soon" tiles for TikTok Ads, Klaviyo). Per-connector card shows the OAuth/plugin/API-key path inline so users know what they're committing to before clicking ([_crosscut_onboarding_ux.md](../competitors/_crosscut_onboarding_ux.md)).
  - **Note on GA4:** GA4 is a first-class integration as of 2026-04. It pulls source/medium/campaign, conversions, totalRevenue, and per-order transactionId via the Analytics Data API. Two nightly jobs: `SyncGA4AttributionJob` (04:45) writes `ga4_daily_attribution`; `SyncGA4OrderAttributionJob` (05:00) writes `ga4_order_attribution`. The `GA4Source` is registered in `AttributionParserService` at priority 5.
  - **Note on Shopify re-auth:** the new `read_analytics` scope is required for Shopify Analytics session data (`shopify_daily_sessions` table, synced by `SyncShopifyAnalyticsJob`). Stores connected before this scope was added will show a warning chip on their connector card until the merchant re-authenticates.
...
- Above-the-fold layout (split 8/4):
  - **Left (8 cols)** — `KpiGrid` (3-up) of per-destination accuracy cards (`MetricCard` variant, source=single, sparkline on):
    ...
    - `GA4 attribution sync accuracy (7d)` · sparkline · SourceBadge=GA4 · tooltip explains the nightly sync cadence and that `ga4_order_attribution` rows are matched to `orders.external_id` via `transaction_id` (match rate surfaced here). Amber chip when match rate < 80%.
