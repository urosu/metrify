...
|---|---|---|---|
| Real | yes | `RevenueAttributionService` computed from all sources; gold lightbulb. Default lens, leftmost badge. | derived on request; recomputes on filter change (Klaviyo-style retroactive recalc, [UX §6](../UX.md#6-interaction-conventions)) |
...
| GSC | no | `SearchConsoleClient` — indirect on Dashboard (used in Revenue overlay via Organic Search channel) | 48h lag (GSC native) |
| GA4 | no | `ga4_daily_attribution` + `ga4_order_attribution` via `GA4Client` Analytics Data API; sessions + source/medium/campaign + per-order via transactionId | <15 min after nightly sync |
...
- `KpiGrid` (4 cols on desktop, 2 on tablet, 1 on mobile) · the six thesis-anchoring cards:
  - `MetricCard` "Revenue" · sources=[Real*, Store, Facebook, Google, GSC, GA4] · value = `daily_snapshots.revenue` (Store total inclusive of tax in reporting currency) · clicking Real → `/orders?source=real`
...
- `StatStripe` · secondary metrics band below KpiGrid, no source badges (Store-sourced only) · 5 stats:
  - **Units Sold** — `daily_snapshots.items_sold` for selected range
  - **CVR** — `NULLIF(sessions, 0) / orders_count`; dash + tooltip "Connect GA4 for CVR" (WooCommerce) or automatically populated from Shopify Analytics (Shopify) when `sessions IS NULL`. Session priority ladder: Shopify Analytics → GA4 → null (see [schema §1.9](../planning/schema.md#19-ga4-full-attribution-source))
  - **Revenue Ex. VAT** — `daily_snapshots.revenue - tax_total` (material for EU/WooCommerce stores where Inc. VAT figure is 20–25% higher)
  - **Shipping Revenue** — `daily_snapshots.shipping_revenue_total` (charged to customer; not the same as `shipping_cost_total` which is cost to store)
  - **Tax Collected** — `daily_snapshots.tax_total`
...
- Two-column region (8/4 split):
  - **Left (8 cols)** — `LineChart` "Revenue over time" · multi-source overlay (Real solid gold, Store slate, Facebook indigo, Google amber, GSC emerald, GA4 violet — [UX §4 source colors](../UX.md#color-tokens)) · `GranularitySelector` defaults Weekly for ≥14d ranges · dotted incomplete-period segment always-on ([UX §5.6](../UX.md#56-chart-primitives)) · `TargetLine` when workspace has a Revenue target set ([UX §5.23](../UX.md#523-target))
...
- **Conversion Funnel** (when sessions data exists) · `FunnelChart` at `Components/charts/FunnelChart.tsx` · 6-col width · three stages: Sessions → Product Views (GA4, optional — only rendered when GA4 is connected and `ga4_daily_attribution` has `page_views` data) → Purchases · source=[GA4 for sessions/product-views, Store for purchases] · hidden entirely when `sessions IS NULL` for the selected range (no sessions source connected)
...
- TrustBar wraps onto two lines (Real / Store / FB on row 1; Google / GSC / GA4 / Not Tracked on row 2).
