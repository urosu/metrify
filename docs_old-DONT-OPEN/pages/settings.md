...
- **Email digest** card:
  ...
  - `Send hour` — hour-of-day in workspace timezone (0–23, default 08; weekly + monthly only). Daily digests send at the configured hour; the workspace `reporting_timezone` determines the effective UTC offset.
  ...
  - **Backed by:** `digest_schedules` table + `CreateDigestScheduleAction`. Route: `PUT /{workspace}/settings/notifications/digest`. One row per workspace; upsert on every save so there is never more than one active schedule per workspace. `DeliverDigestJob` reads `digest_schedules` nightly and respects `frequency` / `day_of_week` / `send_hour`.
