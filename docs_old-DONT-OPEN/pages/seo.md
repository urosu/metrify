...
| GA4 (violet-500) | N/A — greyed on this page | — | — |
| Real (yellow-400) | N/A — greyed on this page (pre-click surface) | — | — |

Notes on source behavior specific to this page:

- Every other SourceBadge (Facebook, Google Ads, GA4, Real) is rendered greyed on every card with tooltip "Not applicable on /seo — SEO is pre-click".
...
- **`ProfitMode` is greyed** with tooltip "Profit requires click + order data — not applicable to pre-click search surface". Same treatment for `AttributionModelSelector`, `WindowSelector`, `AccountingModeSelector`, and the Store/Facebook/Google/GA4/Real badges on each MetricCard.
