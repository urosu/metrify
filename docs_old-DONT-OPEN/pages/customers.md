...
| GA4 / GSC | no | GA4 session data informs acquisition-channel breakdown when GA4Source resolves attribution; GSC not used for customer lifecycle. Both badges greyed on MetricCards. | — |
...
LtvDriversTable (Lifetimely pattern)
  ...
  - Note: `payback_days` in `channel_ltv` is still null in v1 — the column exists in the schema but is not yet populated by any job. The column renders "—" with tooltip "Payback period computation coming in v1.5". Full payback-period calculation is deferred to v1.5.
