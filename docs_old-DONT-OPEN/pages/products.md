...
- Which products will run out of stock in the next 7 / 14 / 30 days?
...
| GA4 (violet-500) | Optional | Product-level attribution from `ga4_daily_attribution` via campaign/source breakdown; ad spend attributed via `parsed_convention` cross-reference | <15 min after nightly sync |
...
- `KpiGrid` (4 cols) — all four cards full-source-badge row (Real + Store primary, Facebook + Google + GA4 for spend/attribution cards):
...
- `KpiGrid` source badge note: the six-source row includes GA4 (violet-500) when GA4 is connected; the former "Site" slot is now GA4. SourceBadge "Site" does not appear on this page in v1.
...
- **Products `DataTable` (§5.5)** — the core of the page. Sticky header, default sort Revenue DESC, default 50 rows, infinite scroll. Columns (column picker §5.5, `SavedView` §5.19):
  ...
  - Velocity (units/day over range; 7-day rolling sparkline column, §5.6 Sparkline) — sourced from `product_variants.velocity_28d`
  - **Stock** — `stock_quantity` from `product_variants` with `StatusDot` (§5.9): emerald >14d cover, amber 3–14d, rose <3d, zinc when stock or velocity is null. Hidden on WooCommerce stores where stock sync is not configured ("N/A" with tooltip).
  - **Days of Cover** — `NULLIF(stock_quantity, 0) / NULLIF(velocity_28d, 0)`, formatted as "23d"; "—" when either value is null. Sortable. This column drives the Stockout Risk filter.
...
- **Row click → `DrawerSidePanel` (§5.10)** — per-product detail:
  ...
  - Inventory snapshot (when store connector exposes stock — Shopify yes, WooCommerce depends on plugin):
    - `StatusDot` (§5.9) for stock health: emerald >14d cover, amber 3–14d, rose <3d, zinc when stock/velocity unknown
    - Units on hand (`product_variants.stock_quantity`)
    - Velocity: `product_variants.velocity_28d` units/day (last 28 days from `order_items`)
    - Days of cover: `stock_quantity / NULLIF(velocity_28d, 0)` — "—" when null
    - "Stock not available from this connector" when the source doesn't expose it (graceful degrade)
  - Margin breakdown:
...
- **`SavedView` examples** expected on this page: `US Rockstars`, `At-Risk EU`, `Missing COGS` (pre-canned filter: COGS is null), `Top 20% by Profit`, `Stockout Risk — 14d` (pre-canned filter: `days_of_cover < 14`, sorted ASC). Seeded on first workspace load. The Stockout Risk view renders a `AlertBanner` (§5.11) at the top of the results: "N products will run out in the next 14 days."
...
- **Inventory forecasting with ML reorder-point suggestions** — seasonality-adjusted demand prediction, reorder quantities, lead-time modeling. Days-of-cover (simple `stock ÷ velocity_28d` arithmetic) **is v1**; what's deferred is the ML layer that projects forward accounting for trend and seasonality (Shopify Advanced / Metorik territory).
- **Product-level first-party pixel / sessions / Add-to-Cart Rate.** The "Site" first-party pixel is dropped from v1. GA4 provides product-level attribution data in v1 via `ga4_daily_attribution`. A dedicated on-site pixel returns in v2 with a new source token (distinct from GA4 — do not collapse them).
