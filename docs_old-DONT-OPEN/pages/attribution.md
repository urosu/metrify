...
|---|---|---|---|
| Real (yellow-400) | Yes — computed. Default lens, leftmost badge. | `RevenueAttributionService` reconciling all sources via workspace model + window | Derivative of inputs |
...
| GSC (emerald-500) | Optional | `search_console_daily_rollups` clicks as an organic-attribution signal; never a conversion source in v1 | 24–48h (Google delay) |
| GA4 (violet-500) | Yes | Full attribution via Analytics Data API; per-order matching via `transaction_id` → `orders.external_id`; populates `ga4_daily_attribution` + `ga4_order_attribution` | <15 min after nightly sync |
...
- **Source Disagreement Matrix** (signature viz, page-local, novel — extends the Rockerbox / Fairing side-by-side pattern to N sources):
  ...
  - Columns = sources (`Real · Store · Facebook · Google · GSC · GA4`). Real is leftmost (default lens).
...
- **Model Comparison Table** (Fairing side-by-side delta pattern, extended; Northbeam Model Comparison promoted out of hamburger menu per anti-pattern):
  ...
  - Columns = attribution models: `First Touch · Last Touch · Last Non-Direct Click` ship in v1. `Linear · Clicks-Only · Clicks + Modeled Views` are deferred (Linear/position-based/time-decay need multi-touch journey reconstruction; Clicks-Only is partial — click IDs captured but ad_insights join not yet wired; Clicks + Modeled Views is v2 requiring Conversions API). Deferred columns render greyed with tooltip stating the reason.
...
- **Site pixel (first-party)** — dropped from v1; GA4 is the v1 replacement for first-party session + attribution data. Site returns in v2 as an on-site pixel with a new color token (suggested teal `#14b8a6`) — do not collapse with GA4.
