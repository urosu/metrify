...
2. **Reconciliation, not truth.** We never claim a number is "real" or "true" as a sales pitch. We label sources and show gaps. (Anti-pattern source: Hyros.)
...
6. **Never empty.** During sync, pre-populated demo data with a "Demo" banner. After sync, real data. Pattern source: [Putler](competitors/_ui_research_putler_roasmonster_atria_motion.md).
7. **Typography as documentation.** Metric suffixes (`ROAS (7d)`, `CAC (LTV)`, `Revenue (blended)`) replace tooltips wherever possible. Pattern source: [Northbeam](competitors/_ui_research_triple_whale_northbeam.md).
...
| 2 | `/profit` | Profit | What's my real P&L after COGS, shipping, fees, taxes, ad spend? |
| 3 | `/customers` | Customers | Who are my customers, and how do they behave? (segments, cohorts, RFM) |
| 4 | `/orders` | Orders | Which orders came in, from where, with what attribution? |
| 5 | `/products` | Products | Which products drive revenue, margin, LTV? |
| 6 | `/inventory` | Inventory | What's my stock health and reorder risk? |
| 7 | `/ads` | Ads | Which campaigns, adsets, ads, creatives are working? |
| 8 | `/attribution` | Attribution | Why don't my platform numbers match my store? |
| 9 | `/seo` | SEO | How am I ranking on Google? (GSC queries, pages, positions) |
| 10 | `/performance` | Performance | How fast is my store? (CWV, TTFB, uptime) |
| 11 | `/integrations` | Integrations | Are my connections healthy, what data am I missing? |
| 12 | `/settings/*` | Settings | Workspace, team, costs (COGS / shipping / fees / VAT), billing, notifications |
| — | `/alerts` | Alerts | What needs my attention right now? |

Auth-only routes (`/auth/*`), onboarding (`/onboarding`), not listed in sidebar.

### Sidebar order

Home · Profit · Customers · Orders · Products · Inventory · Ads · Attribution · SEO · Performance · — · Alerts · Manage (collapsed)

Divider separates "work" from "config". Manage group collapses into Tag Generator, Channel Mappings, Naming Convention, Product Costs, Stores, Holidays.

### Out of scope for v1 (intentional)

| Deferred | Why | Revisit |
|---|---|---|
| Custom SQL report builder | Templated reports solve 90% of use cases. See [Lifetimely role-based templates](competitors/_ui_research_polar_lifetimely.md). | v2 |
...
|---|---|---|---|---|
| Real (Nexstage-computed) | `source-real` | yellow-400 | `#facc15` | Gold lightbulb per thesis. Default lens, leftmost. |
...
| GA4 | `source-ga4` | violet-500 | `#8b5cf6` | Separate from all platforms; replaces former "Site" slot |

The legacy "Site" badge (first-party on-site pixel) was dropped from v1 because the connector won't ship until v2. GA4 takes its color slot. When Site returns it gets a new token (suggested: teal `#14b8a6`) — do not collapse them.
...
Pattern source: [Northbeam](competitors/_ui_research_triple_whale_northbeam.md) — the gold standard.
...
- **Custom date**: when card's date range diverges from page-global, a blue-tinted chip in top-left reads `Card range: Last 7d`. Click → resets to global range. Loud because divergent dates are the #1 "why don't numbers match" support ticket ([Peel](competitors/_ui_research_peel_metorik.md)).
...
Source badges, ProfitMode, Breakdown all still apply. Pattern source: Glew (benchmark). Novel extension: per-workspace sparkline overlay.

### 5.1.2 MetricCardMultiValue

For skewed distributions: three small stacked values side-by-side (Mean / Median / Mode). Used only for AOV on Dashboard, AOP on Profit page, LTV variants on Customers. Never more than three values — anything more goes in a chart. Pattern source: [Triple Whale](competitors/_ui_research_triple_whale_northbeam.md) AOV tile.
...
Used for: per-product COGS, per-campaign convention overrides, per-metric workspace targets, chart annotations. Pattern source: [Linear](competitors/_inspiration_linear.md), Glew (inline COGS on products).
...
| `DaypartHeatmap` | 7×24 grid (rows = day of week, columns = hour-of-day). Cell = metric value (CPA, sales, purchases). Used on `/ads` ad scheduling, `/products` top-10 SKU sales. Pattern source: [Putler](competitors/_ui_research_putler_roasmonster_atria_motion.md). |
| `FunnelChart` | Horizontal bars (rows), each labeled with stage name · absolute count · drop-off % from prior step. Bar width scales with session count. Sticky top bar shows overall conversion. **Never stacked bars for funnels** ([Plausible](competitors/_inspiration_plausible.md)). BigCommerce Purchase Funnel as benchmark for DTC. |
| `QuadrantChart` | Scatter with 1–100 relative indexing ([Northbeam](competitors/_ui_research_triple_whale_northbeam.md) pattern). |
| `LayerCakeChart` | Stacked cohort revenue over time (Daasity pattern). For LTV on Customers page. |

All charts: tooltip on hover, click to filter, right-click opens ContextMenu (§5.21).

**GranularitySelector** — every LineChart includes a local-toolbar switcher: `Hourly · Daily · Weekly · Monthly`. Available granularities depend on date range (hourly only when range ≤ 3 days; monthly only when range ≥ 60 days). Default: **Weekly** when range ≥ 14 days, otherwise Daily. URL-stateful. Pattern source: [Northbeam](competitors/_ui_research_triple_whale_northbeam.md).
...
Pattern source: [Peel](competitors/_ui_research_peel_metorik.md) Annotations.

### 5.7 EmptyState

Three flavors:

- **Syncing**: skeleton + "Importing 4,200 of 18,400 orders (23%) — est. 8 minutes". Pattern source: [Lifetimely](competitors/_ui_research_polar_lifetimely.md). Copy in [`_crosscut_ux_copy.md`](competitors/_crosscut_ux_copy.md).
...
Pattern source: [Putler](competitors/_ui_research_putler_roasmonster_atria_motion.md). Anti-pattern: empty dashboards with dash placeholders ([Triple Whale](competitors/_ui_research_triple_whale_northbeam.md) on first sync).
...
- Pattern sources: [RoasMonster](competitors/_ui_research_putler_roasmonster_atria_motion.md) four-level hierarchy Total → Country → Shop → Product; Shopify Plus ShopifyQL `BY country`.
...
URL-stateful (`?mode=profit`). Off by default. Pattern source: [Lifetimely](competitors/_ui_research_polar_lifetimely.md) role-based dashboards where the CFO template defaults profit-on.
...
- Workspace-shared by default; private views opt-in ([Northbeam](competitors/_ui_research_triple_whale_northbeam.md) pattern).
- Max 12 pinned views per user per page; overflow accessible via Cmd+K.

Pattern source: [Stripe](competitors/_inspiration_stripe.md), [Linear](competitors/_inspiration_linear.md), [Metorik](competitors/_ui_research_peel_metorik.md).
...
Pattern source: [Stripe](competitors/_inspiration_stripe.md) "Live" dot, [Vercel](competitors/_inspiration_vercel.md) relative deploy timestamps. Anti-pattern: [GA4](competitors/_inspiration_ga4.md) silent 24h latency, [Putler](competitors/_ui_research_putler_roasmonster_atria_motion.md) "Pulse" on 15-min-stale data.
...
Pattern source: [Putler](competitors/_ui_research_putler_roasmonster_atria_motion.md) Pulse, [Lifetimely](competitors/_ui_research_polar_lifetimely.md) forecast-as-goal, [Polar](competitors/_ui_research_polar_lifetimely.md) Custom Targets, [RoasMonster](competitors/_ui_research_putler_roasmonster_atria_motion.md).
...
Pattern source: [Putler](competitors/_ui_research_putler_roasmonster_atria_motion.md), [Shopify Native](competitors/shopify-native.md) Live View, [Triple Whale](competitors/_ui_research_triple_whale_northbeam.md) Live Orders. Anti-pattern: real-time branding on delayed data ([Putler](competitors/_ui_research_putler_roasmonster_atria_motion.md)) — freshness pill must be truthful.
...
Pattern source: [Northbeam](competitors/_ui_research_triple_whale_northbeam.md) — the signature differentiator we're copying verbatim.
...
Pattern source: [Varos](competitors/varos.md). Anti-pattern: MonsterInsights deltas on n=3 samples.
...
For entire surfaces that need calibration, an amber AlertBanner (§5.11) announces the state: "Modeled Views are calibrating — 18 of 30 days. Numbers become reliable on May 12." Pattern source: [Northbeam](competitors/_ui_research_triple_whale_northbeam.md) day-30/60/90 framing.
...
Pattern source: [Triple Whale](competitors/_ui_research_triple_whale_northbeam.md), Metabase, Looker Studio. v1 scope per [`_crosscut_export_sharing_ux.md`](competitors/_crosscut_export_sharing_ux.md).

### 5.30 ExportMenu

Primary button label "Export" (never "Download"). Opens dropdown:

- **CSV** — server-rendered, includes six-source columns (`revenue_real`, `revenue_store`, `revenue_facebook`, `revenue_google`, `revenue_gsc`, `revenue_ga4`, `not_tracked`), never collapsed to a single revenue. Download starts immediately if < 10k rows; otherwise opens a "Preparing export" modal with email-on-complete.
...
URL-stateful (`?view=triage`). Default view is page-specified. Pattern source: [Peel](competitors/_ui_research_peel_metorik.md) three-view cohort toggle.
...
Greyed when `ConfidenceChip` threshold unmet. Used on `/ads` Creative Gallery + Triage. Pattern source: [Atria](competitors/_ui_research_putler_roasmonster_atria_motion.md).
...
Pattern source: [Motion](competitors/_ui_research_putler_roasmonster_atria_motion.md) creative-card stat strip.
...
Each chip: label · top 3 values + share of the segment. Used on `/customers` Audiences, `/customers` Customer detail drawer, `/products` drawer (top purchasing segments). Pattern source: [Peel Audience Traits](competitors/_ui_research_peel_metorik.md).
...
Used in table cells on `/orders` (Touchpoints column), `/ads` (AdDetail drawer), `/attribution` (Customer Journey cards), `/customers` (Customer drawer). All four must reference this primitive — no page-local reimplementation. Pattern source: [Northbeam Orders](competitors/_ui_research_triple_whale_northbeam.md).
...
Full definitions: [`_crosscut_metric_dictionary.md`](competitors/_crosscut_metric_dictionary.md). Never call these "True ROAS" — that framing is the Hyros anti-pattern.
...
Currency: single display currency (workspace setting). FX conversion visible on hover ("$12,400 USD · €11,310 native"). Pattern source: [Putler](competitors/_ui_research_putler_roasmonster_atria_motion.md) transaction-date FX.
...
|---|---|---|
| First signup, no sync yet | Demo data + "Demo" banner + progress bar ([Putler](competitors/_ui_research_putler_roasmonster_atria_motion.md)) | [`_crosscut_ux_copy.md`](competitors/_crosscut_ux_copy.md) onboarding section |
| Sync in progress | Skeleton + `"Importing X of Y orders (Z%) — est. N minutes"` ([Lifetimely](competitors/_ui_research_polar_lifetimely.md)) | [`_crosscut_ux_copy.md`](competitors/_crosscut_ux_copy.md) empty-state section |
...
7. **Day 7, 14, 30**: phased-unlock messages ("Cohort analysis unlocks today — you now have enough data"). Pattern source: [Northbeam](competitors/_ui_research_triple_whale_northbeam.md) day-30/60/90 framing.
