...
/**
 * Workspace channel mappings CRUD — a SubNavTab under `/integrations`
 * (docs/planning/frontend.md:53).
 *
 * Renders the workspace + global mapping tables plus the top-20 unclassified
 * (source, medium) pairs from the last 90 days so users can map them directly.
 * Creating/updating/deleting a workspace-scoped override dispatches
 * {@see ReclassifyOrdersForMappingJob} to re-stamp historical orders whose
 * `attribution_last_touch` matches the pair.
 *
 * Reads: channel_mappings, orders (attribution_last_touch).
 * Writes: channel_mappings (workspace-scoped rows).
 * Called by: /integrations/channel-mappings routes.
 *
 * @see docs/planning/backend.md §6 (ManageChannelMappingsController in split plan)
 */
class ChannelMappingsController extends Controller
{
    public function __invoke(Request $request): Response
    {
        ...
        return Inertia::render('Integrations/ChannelMappings', [
     ...
     * rows; workspace overrides are preserved by the seeder's `workspace_id IS NULL`
     * clause. Busts the classifier cache for every workspace.
     */
